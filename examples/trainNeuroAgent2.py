...
def process(state):
    state = state[3]['batched_step_result'].obs[1].reshape(-1,9)
    agent = state[[0,11,22]][:, 0:6].flatten()
    object_last_pos = state[23][:, 3:6].flatten()
    return np.append(agent, object_last_pos)
...
if __name__ == "__main__":
    ...
    state = process(environment.step({'Learner':(0,0)}))
